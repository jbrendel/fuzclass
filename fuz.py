...
class FuzClass(object):
    ...
    """
    def __find_similar_attr(self, name):
        """
        Searching for an attribute that sounds or is similar to
        the specified name.

        We return either the name of an existing attribute
        with a similar name, or - if nothing is found - just
        the name that was specified here.

        'similar' here means: It either sounds similar, or the
        spelling is similar enough (same start and end letter,
        but inner letters can be jumbled).

        """
        # We are not considering really short names or names
        # that start with an underscore. They are - in effect -
        # protected from our shenanigans.
        if name[0] != "_"  and  len(name) >= 2:
            ...
            existing_attrs       = [a for a in dir(self) if a[0] != "_" and len(a) >= 2]
            ...
            for i, attr_lower in enumerate(existing_attrs_lower):
                ...
                # of if the inner letters match closely enough (an extra or missing
                # letter is allowed)
                ...
                if attr_lower == name_lower  or  \
                        (attr_lower[0] == name_lower[0] and
                         ...
                         len(specified_inner_letters.symmetric_difference(attr_inner_letters)) < 2):
                    # Existing lower attrs are in same order as original existing
                    # attrs. Thus, the position of this match is the correct one
                    # to find the original attr name in the original existing attrs
                    # list.
                    return existing_attrs[i]

            # Now check if we find any attributes that at least sound like
            # what we specified.
            name_sound = fuzzy.nysiis(name)  # That's what specified attribute sounds like
            if name_sound:
                for attr in existing_attrs:
                    # Compare the sound of the specified attribute name
                    # with the sounds of any exiting attributes. I guess
                    # we could cache the sounds of the existing attributes
                    # somewhere, if we don't mind spending some RAM on that.
                    existing_attr_name_sound = fuzzy.nysiis(attr)
                    if existing_attr_name_sound == name_sound:
                        # Yay! Found a similar sounding attribute. This
                        # must be what the developer really meant! We
                        # know better... :-)
                        return attr
    ...
    def __setattr__(self, name, value):
        """
        When setting an attribute, we first see whether a similar
        sounding one exists already. If so, we just assign the value
        to that one instead!

        It was probably just a typo, so we're just trying to help out.

        """
        new_name = self.__find_similar_attr(name)
        super(FuzClass, self).__setattr__(new_name, value)

    def __getattr__(self, name):
        """
        Before complaining about not being able to find an attribute,
        we just see if maybe the developer just made a typo. If there
        is an attribute already that sounds similar, we will just
        ...
        if new_name == name:
            ...
            return super(FuzClass, self).__getattribute__(new_name)
